...
function test_string_length_operator()
  local str = "Hello"
  -- much simpler than string.length or length(string)
  assert_equal(5, #str)
end
...
function test_it_doesnt_matter_how_many_equal_signs_you_use_as_long_as_they_are_the_same_at_the_start_and_end()
  -- okay.  a bit overkill.  makes the code harder to read
  local a = [===[one [[two]] three]===]
  local b = "one [[two]] three"
  assert_equal(true, a == b)
end
...
function test_double_bracketed_quoted_str_dont_interpret_escape_characters()
  -- but hey, we have the double brackets for these instead
  local str = [[\n]]
  assert_equal(2, #str)
end
...
function test_dot_dot_will_concatenate_two_strings()
  local str = "Hello, " .. "World"
  --[[ got initially stumped as local font in editor made it look very similar to lowercase
Have now switched local font
]]
...
function test_numbers_must_be_converted_to_strings_before_concatenation()
  ...
  assert_equal('Lua is 21 years old', str)
end
